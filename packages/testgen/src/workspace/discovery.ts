...
export function resolveWorkspacePackages(config: TestgenConfig, rootDir: string = ROOT_DIR): ResolvedPackage[] {
...
export function resolveTargetFiles(options: ResolveTargetFilesOptions): TargetFile[] {
    ...
    for (const filePath of deduped) {
        ...
        const framework = options.frameworkOverride && options.frameworkOverride !== 'auto'
...
function resolvePackage(pkg: TestgenPackageConfig, defaults: TestgenConfig['defaults'], rootDir: string): ResolvedPackage {
...
function filterSelectedPackages(packages: ResolvedPackage[], packageName?: string): ResolvedPackage[] {
...
function matchesPackageGlobs(filePath: string, pkg: ResolvedPackage): boolean {
    ...
    const includeMatch = pkg.include.length === 0 || pkg.include.some((pattern) => matchGlob(relativePath, pattern));
...
function resolveRenderHelperPath(renderHelper: string | 'auto', packageRoot: string): string | 'auto' {
