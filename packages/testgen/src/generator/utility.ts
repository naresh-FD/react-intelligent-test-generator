        ...
        if (parentName) {
            ...
            for (const func of funcs) {
                if (/^use[A-Z]/.test(func.name)) {
                    generateHookTests(lines, func, sourceFile, '  ', needsQueryClient);
                } else {
...
function generateObjectMethodTests(lines: string[], func: ExportedFunction, sourceFile: SourceFile, indent: string): void {
    ...
    if (func.isAsync) {
        ...
        if (isVoid) {
            lines.push(`${indent}    await expect(${callExpr}(${mockArgs})).resolves.not.toThrow();`);
        } else {
    ...
    if (requiredParams.length < func.params.length) {
        ...
        if (func.isAsync) {
            ...
            if (isVoid) {
                lines.push(`${indent}    await expect(${callExpr}(${minArgs})).resolves.not.toThrow();`);
            } else {
    ...
    for (const param of func.params.slice(0, 3)) {
        ...
        for (const edgeCase of edgeCases) {
            ...
            if (func.isAsync) {
                ...
                if (isVoid) {
                    lines.push(`${indent}    expect(() => ${callExpr}(${argsWithEdge})).not.toThrow();`);
                } else {
...
function generateFunctionTests(lines: string[], func: ExportedFunction, sourceFile: SourceFile, indent: string): void {
    ...
    if (func.isAsync) {
        lines.push(`${indent}  it("resolves without throwing", async () => {`);
        if (isVoid) {
            lines.push(`${indent}    await expect(${func.name}(${mockArgs})).resolves.not.toThrow();`);
        } else {
    ...
    if (requiredParams.length < func.params.length) {
        ...
        if (func.isAsync) {
            lines.push(`${indent}  it("works with minimal arguments", async () => {`);
            if (isVoid) {
                lines.push(`${indent}    await expect(${func.name}(${minArgs})).resolves.not.toThrow();`);
            } else {
    ...
    for (const param of func.params.slice(0, 3)) {
        ...
        for (const edgeCase of edgeCases) {
            ...
            if (func.isAsync) {
                lines.push(`${indent}  it("handles ${edgeCase.label} for ${param.name}", async () => {`);
                if (isVoid) {
                    lines.push(`${indent}    await expect(${func.name}(${argsWithEdge})).resolves.not.toThrow();`);
                } else {
                ...
                lines.push(`${indent}  it("handles ${edgeCase.label} for ${param.name}", () => {`);
                if (isVoid) {
                    lines.push(`${indent}    expect(() => ${func.name}(${argsWithEdge})).not.toThrow();`);
                } else {
