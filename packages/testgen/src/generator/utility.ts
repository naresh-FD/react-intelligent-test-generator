...
export function generateUtilityTest(
    ...
    const needsAxiosMock = sourceText.includes('axios') || sourceText.includes("from 'axios'") || sourceText.includes('from "axios"');
    const needsApiMockOnly = !needsAxiosMock && apiImportPath !== null && fileType === 'service';
    const needsFetchMock = sourceText.includes('fetch(') && !sourceText.includes('useFetch');
    ...
    if (needsAxiosMock) {
        ...
        lines.push(`    patch: ${mockFn()}.mockResolvedValue({ data: {} }),`);
        lines.push(`    defaults: { headers: { common: {} } },`);
        lines.push(`    interceptors: { request: { use: ${mockFn()} }, response: { use: ${mockFn()} } },`);
        lines.push('  },');
        lines.push('  create: () => mockAxiosInstance,');
        lines.push(`  get: ${mockFn()}.mockResolvedValue({ data: {} }),`);
        lines.push(`  post: ${mockFn()}.mockResolvedValue({ data: {} }),`);
    ...
    const needsURLMock = sourceText.includes('URL.createObjectURL') || sourceText.includes('URL.revokeObjectURL');
    ...
    if (needsLocalStorageMock) {
        ...
        lines.push('    value: {');
        lines.push(`      getItem: ${mockFn()}.mockImplementation((key: string) => mockStorage[key] ?? null),`);
        lines.push(`      setItem: ${mockFn()}.mockImplementation((key: string, val: string) => { mockStorage[key] = val; }),`);
        lines.push(`      removeItem: ${mockFn()}.mockImplementation((key: string) => { delete mockStorage[key]; }),`);
        lines.push(`      clear: ${mockFn()}.mockImplementation(() => { Object.keys(mockStorage).forEach(k => delete mockStorage[k]); }),`);
        lines.push('      length: 0,');
    ...
    const importNames = [
        ...directExports.map(f => f.name),
        ...Array.from(objectExports),
    ];
    ...
    if (hasHooks) {
        ...
        if (useRenderWithProviders) {
            ...
            lines.push('  const queryClient = createTestQueryClient();');
            lines.push('  return ({ children }: { children: React.ReactNode }) =>');
            lines.push('    React.createElement(QueryClientProvider, { client: queryClient }, children);');
            lines.push('}');
        }
        lines.push('');
    }
 ...
 */
function generateObjectMethodTests(lines: string[], func: ExportedFunction, sourceFile: SourceFile, indent: string): void {
...
function generateFunctionTests(lines: string[], func: ExportedFunction, sourceFile: SourceFile, indent: string): void {
    lines.push(`${indent}describe("${func.name}", () => {`);
    ...
    for (const switchCase of switchCases) {
        for (const caseValue of switchCase.values) {
            const argsWithCase = func.params.map((p) =>
                p.name === switchCase.paramName ? caseValue : mockValueForParam(p)
            ).join(', ');
...
function generateHookTests(lines: string[], func: ExportedFunction, sourceFile: SourceFile, indent: string, needsQueryClient = false): void {
    lines.push(`${indent}describe("${func.name}", () => {`);
    ...
    if (func.params.length > 0) {
        ...
        lines.push(`${indent}      (props) => ${func.name}(${func.params.map((_, i) => `props.p${i}`).join(', ')}),`);
        lines.push(`${indent}      { initialProps: { ${func.params.map((p, i) => `p${i}: ${mockValueForParam(p)}`).join(', ')} }${rerenderWrapperOpt} }`);
        lines.push(`${indent}    );`);
        lines.push(`${indent}    rerender({ ${func.params.map((p, i) => `p${i}: ${rerenderArgs.split(', ')[i] || mockValueForParam(p, true)}`).join(', ')} });`);
        lines.push(`${indent}    expect(result.current).toBeDefined();`);
        lines.push(`${indent}  });`);
        lines.push('');
    }
...
function detectHookReturnShape(func: ExportedFunction, sourceFile: SourceFile): HookReturnShape {
    const shape: HookReturnShape = { type: 'unknown', functionIndices: [], methods: [], stateProps: [] };
    ...
    const returnStatements = funcNode.getDescendantsOfKind(SyntaxKind.ReturnStatement).filter(ret => {
        ...
        while (parent && parent !== funcNode) {
            ...
            if (Node.isArrowFunction(parent) || Node.isFunctionExpression(parent) ||
                Node.isMethodDeclaration(parent) || Node.isFunctionDeclaration(parent)) {
                return false;
            }
    ...
    for (const ret of returnStatements) {
        ...
        if (Node.isArrayLiteralExpression(expr)) {
            ...
            elements.forEach((el, idx) => {
                ...
                if (/^set[A-Z]/.test(text) || /^(handle|toggle|remove|clear|reset|add|update|delete|fetch|load|save|dispatch)/.test(text)) {
                ...
                if (Node.isIdentifier(el)) {
                    ...
                    if (/^set[A-Z]/.test(name) || /^remove/.test(name) || /^toggle/.test(name) || /^clear/.test(name)) {
...
function detectSwitchCases(func: ExportedFunction, sourceFile: SourceFile): SwitchCaseInfo[] {
    ...
    for (const switchStmt of switchStatements) {
        ...
        const matchedParam = func.params.find(p =>
...
function mockValueForParam(param: ParamInfo, alternate = false): string {
    ...
    if (/^(fn|func|callback|handler|listener|predicate|comparator|reducer|mapper|transformer)$/i.test(name)) return mockFn();
    ...
    if (type === 'date' || type === 'Date') return alternate ? 'new Date("2025-06-15")' : 'new Date("2024-01-01")';
    ...
    if (/amount/i.test(name) || /price/i.test(name) || /value/i.test(name)) return alternate ? '200' : '100';
...
function contextualMock(name: string, type: string, alternate = false): string {
    if (type === 'string') {
        ...
        if (/type/i.test(name) || /kind/i.test(name) || /status/i.test(name) || /mode/i.test(name)) return alternate ? '"secondary"' : '"primary"';
        if (/query/i.test(name) || /search/i.test(name) || /filter/i.test(name)) return alternate ? '"other"' : '"test"';
 ...
 */
function detectHookProviderImports(sourceFile: SourceFile, sourceText: string, testFilePath: string): string[] {
    ...
    for (const ctx of contextMap) {
        if (sourceText.includes(ctx.pattern) && !imported.has(ctx.providerName)) {
            ...
            for (const imp of sourceFile.getImportDeclarations()) {
                ...
                for (const named of imp.getNamedImports()) {
                    if (named.getName() === ctx.pattern || named.getName() === ctx.providerName) {
                        ...
                        const sourceDir = pathModule.dirname(sourceFile.getFilePath());
                        ...
                        if (moduleSpec.startsWith('@/') || moduleSpec.startsWith('~/')) {
                            ...
                            while (srcRoot && !require('fs').existsSync(pathModule.join(srcRoot, aliasPath + '.ts')) &&
                                   !require('fs').existsSync(pathModule.join(srcRoot, aliasPath + '.tsx')) &&
                                   !require('fs').existsSync(pathModule.join(srcRoot, aliasPath, 'index.ts')) &&
                                   !require('fs').existsSync(pathModule.join(srcRoot, aliasPath, 'index.tsx'))) {
                                const parent = pathModule.dirname(srcRoot);
                                if (parent === srcRoot) break;
                                srcRoot = parent;
                            }
                            const resolvedPath = pathModule.resolve(srcRoot, aliasPath);
                            let relativePath = pathModule.relative(testDir, resolvedPath).replace(/\\/g, '/');
                        ...
                        const resolvedPath = pathModule.resolve(sourceDir, moduleSpec);
                        let relativePath = pathModule.relative(testDir, resolvedPath).replace(/\\/g, '/');
                        if (!relativePath.startsWith('.')) relativePath = './' + relativePath;
                        relativePath = relativePath.replace(/\.(tsx?|jsx?)$/, '');
                        imports.push(`import { ${ctx.providerName} } from "${relativePath}";`);
            ...
            if (!imported.has(ctx.providerName)) {
                ...
                const testDir = require('path').dirname(testFilePath);
                const possiblePaths = [
                ...
                for (const pp of possiblePaths) {
                    const absPath = require('path').resolve(testDir, pp);
                    const extensions = ['.tsx', '.ts', '.jsx', '.js'];
                    for (const ext of extensions) {
                        try {
                            if (require('fs').existsSync(absPath + ext)) {
